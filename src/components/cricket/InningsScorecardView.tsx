...
function TableHeader({
  ...
  cols: { label: string; width: number; align?: "left" | "center" }[];
}) {
  return (
      ...
      <View style={styles.section}>
        ...
        {data.batters.map((b) => (
          <View key={b.playerId} style={styles.row}>
            <View style={[styles.batterCell, { width: 148 }]}>
              ...
              <Text
                style={[styles.dismissal, !b.isOut && styles.battingTag]}
                numberOfLines={2}
              >
                {b.dismissal}
              </Text>
        ...
        <View style={[styles.summaryRow, styles.totalRow]}>
          ...
          <Text style={styles.totalValue}>
            {data.runs}-{data.wickets} ({data.oversLabel} Overs, RR:{" "}
            {data.runRate})
          </Text>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        {data.bowlers.length === 0 ? (
          ...
          data.bowlers.map((b) => (
            <View key={b.playerId} style={styles.row}>
              <Text
                style={[styles.bowlerName, { width: 100 }]}
                numberOfLines={1}
              >
                {b.name}
              </Text>
